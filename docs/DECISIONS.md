...
**Why self-hosted, not hotlinked:** the section claims "no tracking involved" — hotlinking third-party og:images would leak every visitor's IP to arbitrary hosts and contradict the studio's privacy-first posture. Self-hosting also survives image link rot. ~~`public/trends/` is wiped and rebuilt each run~~ **Correction (2026-07-18):** wiping broke `/journal/<old-date>` pages (their thumbnails 404'd). The directory is now **never wiped**; retention is bounded by **orphan pruning** — after each enrich run, `pruneOrphanThumbs()` deletes only `*.webp` files referenced by no journal JSON and not in the current batch. Referenced history lives as long as its journal entry; renamed/removed items get reclaimed. Measured growth ≈ 130–150 KB/day (~55 MB/yr), acceptable for the static repo. Committed digest JSONs also **strip `imageUrl`** (`stripInternalFields()` in pipeline.mjs) so third-party CDN URLs used during enrichment never leak into the repo — only the self-hosted `image` path is published.
...
2. **`AssociateNotEligible` (was expected, now past-window and unresolved):** even after the `resources` fix, `GetItems` 403s with `{"reason":"AssociateNotEligible"}`. Per Amazon's own FAQ this is expected for up to 48 hours after credential creation (credential created 2026-07-15 ~16:36 UTC). Re-tested 2026-07-17 (past the 48h window): **still 403 `AssociateNotEligible`**. `build-catalog.mjs` handles this correctly (logs, leaves the cache untouched, exits 0) — but the 48h explanation no longer applies, so this is now an owner action: check actual eligibility in Associates Central directly (could be a real qualification gap, not a timing artifact).
...
## D-022 — Signals Journal auto-publishes daily to /journal/<date> (direct commit, no PR)

**Decision:** The daily trends pipeline commits each day's entry **directly to `main`** so it auto-publishes to its own permanent URL (`/journal/YYYY-MM-DD`) with no human PR merge. `trends-digest.yml` now mirrors `globe-data-digest.yml`: `contents: write`, a `trends-digest` concurrency group, `checkout ref: main`, and a `git status --porcelain` guard → `git push origin main` with rebase-and-retry on rejection. Supersedes **D-012's "merge = publish"** model for the journal (D-011's "trend → draft idea → human review → post" still governs *blog posts* via `draft-post.mjs`, which is unchanged and still human-authored).

**Why:** D-012 chose a PR gate to keep a *weekly* solo-owner review low-friction. Two later changes inverted that calculus: D-017 moved the cadence to **daily** (7× the merge friction), and the storage migrated from a single overwritten `trends.json` to a **persistent per-day archive** where each day is its own permanent page. Under a PR gate, any day the owner doesn't merge is a permanent hole in the archive — directly contradicting the "every day saved at its own URL" purpose. The globe data pipeline (D-019) had already dropped its PR gate for direct-commit; the trends storage migration left the *workflow* PR-gated, an inconsistency the owner flagged.

**Editorial-risk tradeoff (accepted):** Unlike globe (numeric weather/TLE data), trends publishes **scraped free-text titles from strangers** under the studio brand with no pre-publish human glance. Mitigations, in order: (1) the staged gate pipeline (`pipeline.mjs`) with its 13-test regression suite runs *before* publish and aborts the run on failure — relevance floor, off-topic/staleness rejection, dedupe, per-pillar/author/total diversity caps; (2) `build-digest.mjs` skips writing an empty day, so a total-scrape-failure never publishes a blank page; (3) corrections are a one-line follow-up commit (edit/revert `src/content/journal/<date>.json` on main) — the archive is plain committed files, nothing is irreversible. The pillar-grouped breakdown that used to be the PR body is now attached to each Actions run via `$GITHUB_STEP_SUMMARY`, preserving the "skim what shipped" visibility without the merge step.

**Implementation:**
- `.github/workflows/trends-digest.yml` — dropped `pull-requests: write` and all `gh pr` mechanics; added concurrency + `ref: main`; publish step commits `src/content/journal/` (+ `public/trends`) directly to main; final step cats `trend-digest-summary.md` into the run step summary.
- `scripts/trends/build-digest.mjs` — writes no `OUT_PATH` when `items.length === 0` (empty-day guard); summary copy reworded from "review before merging" to "auto-published"; header comment updated.
- `src/data/trends.ts`, `src/pages/resources.astro` — comments corrected from PR-review language to auto-publish.
