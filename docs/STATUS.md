...
- **Amazon Associates:** Curated picks in `affiliate-links.json` (tag `engineersport-20`), Creators API catalog cache, grouped gear on `/resources#gear`. Disclosure + `rel="sponsored"`. ASIN integrity audit in `npm run check`.
- **SparkFun Affiliate:** code `rOtrc44SZw` — 14 products (Pi 5, RedBoard, Teensy, Jetson, ZED-F9P, XRP, SIK, IMUs, …). `merchant: "sparkfun"` rows; `buildSparkFunUrl()` appends `?ref=`. **10% on Originals only** (documented in `/privacy#affiliates`).
