...
- **Playground:** `/playground` — unified app-grounded tools and exploratory math/physics toys, labeled honestly per entry. Live: quaternion ↔ Euler converter, Fourier epicycles, Live Earth, Double Pendulum + Phase Space (fixed-step RK4, draggable bobs, twin-trajectory chaos comparison, energy diagnostics), Chladni Cymatics (driven modal superposition with movable driver, frequency sweep, and Grabec-style grain transport), and N-Body Orbital Choreography (Yoshida-4 symplectic integrator, figure-eight and other validated presets, ghost-twin perturbation racing — both added 2026-07-16). Registry: `src/data/playground.ts`.
