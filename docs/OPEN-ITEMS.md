...
- [x] **Gear list expanded to 50 owner-confirmed picks (2026-07-15):** sourced from the owner's own `makersportal.com` posts (BLExAR's 33 electronics items — Arduino/RPi/sensors/motors, all traceable to a real blog post) plus confirmed real gear for Biquadia (6, audio interfaces/mics), nymic (3, USB mics), akous (3, headphones/wireless mic), MotionLink (1, AirPods Pro), itria (3, ML textbooks), and 2 general studio items — all live in `src/data/affiliate-links.json` with real Associates links (`docs/AFFILIATE-CANDIDATES.md` has the full sourcing trail). `/resources#gear` now renders these grouped by app in collapsible sections (33 items in one flat list was unreadable — see `src/pages/resources.astro`'s `gearGroups` helper). itria/GridVerse/Thumb-Dash/PopCloset still have no dedicated gear beyond the shared/general items — fine, don't force a tie-in.
- [ ] **Re-test Creators API eligibility after ~2026-07-17:** `GetItems` was still 403ing with `AssociateNotEligible` as of the 50-item population (2026-07-15, same day as credential creation) — this is expected per Amazon's 48-hour review window (see D-015/D-016), not a bug. Run `node --env-file=.env scripts/amazon/build-catalog.mjs` again; if it still 403s past 2026-07-17, check actual eligibility status directly in Associates Central rather than assuming it's just the timer.
- [ ] **Tabled (2026-07-15): daily trend cadence + trend-driven gear re-ranking.** User wants trends tracked more frequently and the affiliate page to feel "live"/responsive to what's trending. Discussed but deferred pending a dedicated monetization-strategy session — see `docs/HANDOFF-MONETIZATION.md`. Two different risk profiles: bumping `trends-digest.yml`'s cron to daily is cheap/low-risk; using trends to auto-*select which products get recommended* is explicitly NOT what to build (see D-015's "Do not" — no `SearchItems`-based auto-discovery without a human-review gate). The safer version discussed: use the already-computed trending pillars to re-rank/feature which of the 50 curated gear items surfaces first on `/resources#gear`, reusing the `pillars` field already added in D-015 — zero new fabrication risk, not yet built.
