...
- [x] **Daily trend cadence + trend-driven gear re-ranking (D-017, 2026-07-15):** DONE — `trends-digest.yml` bumped to daily `0 14 * * *`, `resources.astro` now re-ranks gearGroups by `pillarCounts` (aggregate `linkTrendScore`), trending chips `↗ {Pillar}` and `Re-ranked · trending …` header, verified 1724px gear height vs 5500px incident, BLExAR stays collapsed. Safe — same 50 ASINs, only sort order changes, no auto-discovery (D-015 Do-not still enforced).
