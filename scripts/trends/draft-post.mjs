 ...
 * workflow/cron. Takes a selected trends.json item and scaffolds a draft
 * post in src/content/blog/ for a human to actually write.
 *
...
const TRENDS_PATH = path.join(ROOT, 'src/data/trends.json');
const APPS_PATH = path.join(ROOT, 'src/data/apps.ts');
const BLOG_DIR = path.join(ROOT, 'src/content/blog');

function loadTrends() {
  const raw = JSON.parse(readFileSync(TRENDS_PATH, 'utf8'));
  return raw.items ?? [];
}
...
function main() {
  ...
  if (args.list || !args.id) {
    console.log('Candidate trend items (src/data/trends.json):\n');
