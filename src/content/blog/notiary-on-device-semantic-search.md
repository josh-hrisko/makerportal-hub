...
Want the packaged version instead of the pattern? The [CoreML Offline Classifier Starter](/shop) bundles a SwiftUI + Vision + CoreML starting point based on the same PopCloset/Notiary on-device approach. For everything else in [`on-device-ai`](/journal), the byte-math above applies to any embedding model you're sizing for a phone, not just this one.
